...
class phpMorphy_Fsa_Sparse_Shm extends phpMorphy_Fsa
{
    public function walk($trans, $word, $readAnnot = true)
    {
        ...
        for ($i = 0, $c = $GLOBALS['__phpmorphy_strlen']($word); $i < $c; $i++) {
            ...
            [, $trans] = unpack('V', shmop_read($__shm, $__offset + ($fsa_start + (((($trans >> 10) & 0x3FFFFF) + $char + 1) << 2)), 4));
        ...
        if ($i >= $c) {
            ...
            if ($readAnnot) {
                ...
                [, $trans] = unpack('V', shmop_read($__shm, $__offset + ($fsa_start + ((($trans >> 10) & 0x3FFFFF) << 2)), 4));
    ...
    public function readState($index)
    {
        ...
        [, $trans] = unpack('V', shmop_read($__shm, $__offset + ($start_offset), 4));
        ...
        foreach ($this->getAlphabetNum() as $char) {
            ...
            [, $trans] = unpack('V', shmop_read($__shm, $__offset + ($start_offset + (($char) << 2)), 4));
    ...
    protected function readRootTrans()
    {
        ...
        [, $trans] = unpack('V', shmop_read($__shm, $__offset + ($fsa_start + 4), 4));
